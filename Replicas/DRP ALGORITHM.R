...
packages <- c("RPostgreSQL","rlang", "ggplot2", "caret", "class", "mapview",  "compare", "pracma" , "stringr","SpatialTools","matlib", "dplyr","chron","lubridate","zoom","RgoogleMaps","ggmap") #librerias 
ipak(packages)  #llama las librerias que se requeriran para el correcto funcionamiento del algortimo

#Datos Globales
carpeta<- "PRUEBA DRP"								                                        #Directorio experimentos
algoritmo<- "DRP"						                              #Algorithm
...
if (dataset == 'Guayaquil') {
  
  time_in_timestamp = 'TRUE'
  data <-dbGetQuery(con, "SELECT longitud as longitude, latitud as latitude, fecha as unixtime, id as file_name FROM ecuadortotal LIMIT 100")
  
}else if (dataset == 'Brasil') {
  
  time_in_timestamp = 'FALSE'
  data <-dbGetQuery(con, "SELECT longitud as longitude, latitud as latitude, fecha as unixtime, id as file_name FROM brasiltotal limit 150")
  
}else if (dataset == 'Sanfrancisco') { 
  
  time_in_timestamp = 'TRUE'
  data <-dbGetQuery(con, "SELECT longitud as longitude, latitud as latitude, fecha as unixtime, id as file_name FROM sanfranciscototal LIMIT 200")
  
}else if (dataset == 'Beijing') { 
  
  time_in_timestamp = 'FALSE'
  data<-dbGetQuery(con, "SELECT longitud as longitude, latitud as latitude, fecha as unixtime, id as file_name FROM beijingtotal LIMIT 5000")
  
}else if (dataset == 'Circulartotal') { 
...
}else if (dataset == 'Circularlineal') {
...
crear_directorio(folders = carpeta, algorithm = algoritmo, dataset = dataset)#Crea el directorio donde se alojaran los resultadso del algortimo
...
for (j in 1:as.numeric(cant_ejecuciones)) {
  ...
  result[[j]]<-  drp(points = lista_trayectorias[[j]], dist_threshold = dist_threshold[[j]])
